...
export interface UserGatewaySession {
  ...
  preflightVoiceJoin(guildId: string, channelId: string): Promise<VoiceJoinPreflight>;
  sessionSnapshot(): GatewaySessionSnapshot;
}

class UserGatewaySessionImpl implements UserGatewaySession {
  ...
  public async preflightVoiceJoin(guildId: string, channelId: string): Promise<VoiceJoinPreflight> {
    ...
    if (!guild) {
      return {
        channelType: null,
        warnings: ['guild_metadata_unavailable'],
      };
    }
    ...
    if (!channel) {
      return {
        channelType: null,
        warnings: ['channel_metadata_unavailable'],
      };
    }

    if (channel.type === STAGE_CHANNEL_TYPE) {
      throw new AppError('Stage voice channels are not supported.', ExitCode.Config, {
        reason: 'unsupported_stage_channel',
        guildId,
        channelId,
      });
    }
    ...
    if (!SUPPORTED_VOICE_CHANNEL_TYPES.has(channel.type)) {
      throw new AppError('The target channel is not voice-capable.', ExitCode.Config, {
    ...
    if (permissions) {
      if (!permissions.connect) {
        warnings.push('missing_connect_permission');
      }

      if (!permissions.stream) {
        warnings.push('missing_stream_permission');
      }
    } else {
  ...
  private async waitForReady(timeoutMs = READY_TIMEOUT_MS): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        cleanup();
        reject(
          new AppError('Timed out waiting for the Discord gateway READY event.', ExitCode.Auth, {
            timeoutMs,
          })
        );
      }, timeoutMs);
  ...
  private async fetchGatewayUrl(): Promise<string> {
    const response = await fetch(`https://discord.com/api/v${GATEWAY_VERSION}/gateway`, {
      method: 'GET',
    });

    if (!response.ok) {
      throw new AppError('Failed to fetch the Discord gateway URL.', ExitCode.Auth, {
    ...
    if (!payload.url) {
      throw new AppError('Discord did not return a gateway URL.', ExitCode.Auth);
    }
  ...
  private connect(url: string, resume: boolean): void {
    ...
    this.webSocket.addEventListener('open', () => {
      this.logger.info('Gateway websocket opened', {
        resume,
        sessionId: this.gatewaySessionId,
      });
    });
      ...
      void this.handleMessage(event.data).catch((error) => {
        const appError =
          error instanceof AppError
            ? error
            : new AppError(
                error instanceof Error ? error.message : 'Gateway message handling failed.',
                ExitCode.Gateway
              );
        this.emitFatal(appError);
      });
    });
  ...
  private handleHello(data: unknown): void {
    ...
    if (!heartbeatInterval) {
      throw new AppError(
        'Discord gateway hello payload was missing heartbeat_interval.',
        ExitCode.Gateway
      );
    }
  ...
  private sendIdentify(): void {
    if (!this.token) {
      throw new AppError('The user gateway session is missing its token.', ExitCode.Auth);
    }
  ...
  private sendResume(): void {
    if (!this.token || !this.gatewaySessionId) {
      throw new AppError(
        'The user gateway session cannot resume without a token and session id.',
        ExitCode.Gateway
      );
    }
  ...
  private emitFatal(error: AppError): void {
    this.logger.error('Gateway session failed', {
  ...
  private mergeGuild(data: unknown): void {
    ...
    for (const channel of guild.channels ?? []) {
      ...
      if (parsed) {
        snapshot.channels.set(parsed.id, parsed);
      }
    }
...
export function createUserGatewaySession(logger: Logger): UserGatewaySession {
  return new UserGatewaySessionImpl(logger);
}

function parseChannelSnapshot(data: unknown, guildId: string): GatewayChannelSnapshot | null {
  const channelId = getStringField(data, 'id');
  const type = getNumberField(data, 'type');
  if (!channelId || typeof type !== 'number') {
    return null;
  }
