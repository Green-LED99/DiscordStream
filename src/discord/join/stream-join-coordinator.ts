...
import type { StreamConnection } from '../voice/stream-connection.js';
import type { JoinAttemptDiagnostics, StreamHandshakeState } from './types.js';

const INITIAL_STREAM_ATTEMPTS = 3;
const HANDSHAKE_TIMEOUT_MS = 8_000;

type StreamJoinCoordinatorOptions = {
...
export class StreamJoinCoordinator {
  ...
  public constructor(
    ...
    private readonly options: StreamJoinCoordinatorOptions = {}
  ) {}

  public async connectInitial(): Promise<WebRtcConnectionWrapper> {
    let lastError: AppError | null = null;
    const initialAttempts = this.options.initialAttempts ?? INITIAL_STREAM_ATTEMPTS;
      ...
      try {
        ...
        lastError =
          error instanceof AppError
            ? error
            : new AppError('Stream create failed.', ExitCode.Gateway);
    ...
    throw lastError ?? new AppError('Stream create failed.', ExitCode.Gateway);
  }
  ...
  private async runAttempt(attempt: number): Promise<WebRtcConnectionWrapper> {
    ...
    const streamKey = generateStreamKey('guild', this.guildId, this.channelId, this.userId);
  ...
  private createTimeoutError(attempt: number): AppError {
    const diagnostics: JoinAttemptDiagnostics = {
      attempt,
      reason:
        !this.handshake.sawStreamCreate && !this.handshake.sawStreamServer
          ? 'join_timeout_no_gateway_response'
          : this.handshake.sawStreamCreate
