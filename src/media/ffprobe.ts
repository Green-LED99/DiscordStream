...
export async function probeMedia(ffprobePath: string, url: string): Promise<FfprobeResult> {
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once('exit', (code) => {
      ...
      reject(
        new AppError('ffprobe failed for the requested media URL.', ExitCode.Media, {
          code,
    ...
    child.once('error', (error) => {
      reject(
        new AppError('Unable to start ffprobe.', ExitCode.Media, {
