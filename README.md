...
`npm run build:libdave` clones the official `discord/libdave` repo, patches its `cpp/CMakeLists.txt` to export `HEAPU8` and `wasmMemory`, runs the upstream wasm configure/build commands directly, writes artifacts into [vendor/libdave](/Users/harrisonpope/Desktop/DiscordStream/vendor/libdave), and then runs `npm run verify:libdave`. The runtime loader in [src/dave/libdave.ts](/Users/harrisonpope/Desktop/DiscordStream/src/dave/libdave.ts) requires `libdave.js` and `libdave.wasm` to exist there.

Run `npm run verify:libdave` if you need to re-check an already-built module. If `HEAPU8` or `wasmMemory` are missing, treat that as a `libdave` artifact/build issue first, not an MLS or voice-protocol issue. If `EMSDK` is missing, `npm run build:libdave` cannot run and the DAVE runtime remains blocked.
